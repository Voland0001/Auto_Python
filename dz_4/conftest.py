...
def create_bad_archive():
    ssh_checkout(data["ip"], data["user"], data["passwd"],
                 f'cd {data["folder_in"]}; 7z a -t{data["arc_type"]} {data["folder_out"]}/arx2', "Everything is Ok")
    ssh_checkout(data["ip"], data["user"], data["passwd"],
                 f'cp {data["folder_out"]}/arx2.{data["arc_type"]} {data["folder_bad"]}', '')
    ssh_checkout(data["ip"], data["user"], data["passwd"],
                 f'truncate -s 1 {data["folder_bad"]}/arx2.{data["arc_type"]}', '')  # сделали битым
...
def stat_fixture():
    ...
    ssh_checkout(data["ip"], data["user"], data["passwd"],
                 f'cd {data["stat_file_dir"]}; echo "time = {time_stamp}, files_count = {files_count}, file_size = {file_size}, cpu_stat = {cpu_stat}" >> stat.txt',
                 '')
