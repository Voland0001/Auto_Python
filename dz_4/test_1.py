...
class TestPositive:
    ...
    def test_add_archive(self, make_folder, clear_folder, make_files, start_time):  # a создали архив
        res_add = ssh_checkout(data["ip"], data["user"], data["passwd"],
                               f'cd {data["folder_in"]}; 7z a -t{data["arc_type"]} {data["folder_out"]}/arx2',
                               "Everything is Ok")
        res_ls = ssh_checkout(data["ip"], data["user"], data["passwd"], f'ls {data["folder_out"]}',
                              f'arx2.{data["arc_type"]}')
        self.save_log(start_time, "log_positive.txt")
    ...
    def test_check_e_extract(self, clear_folder, make_files, start_time):  #
        ...
        res.append(ssh_checkout(data["ip"], data["user"], data["passwd"],
                                ...
                                "Everything is Ok"))
        res.append(ssh_checkout(data["ip"], data["user"], data["passwd"],
                                f'cd {data["folder_out"]}; 7z e arx2.{data["arc_type"]} -o{data["folder_ext"]} -y',
                                "Everything is Ok"))
    ...
    def test_check_x_extract_subfolder(self, clear_folder, make_files, make_subfolder, start_time):
        ...
        res.append(ssh_checkout(data["ip"], data["user"], data["passwd"],
                                ...
                                "Everything is Ok"))
        res.append(ssh_checkout(data["ip"], data["user"], data["passwd"],
                                f'cd {data["folder_out"]}; 7z x arx2.{data["arc_type"]} -o{data["folder_ext"]} -y',
                                "Everything is Ok"))
        for item in make_files:
            res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], f'ls {data["folder_ext"]}', item))

        res.append(
            ssh_checkout(data["ip"], data["user"], data["passwd"], f'ls {data["folder_ext"]}', make_subfolder[0]))
        res.append(
            ssh_checkout(data["ip"], data["user"], data["passwd"], f'ls {data["folder_ext"]}/{make_subfolder[0]}',
                         make_subfolder[1]))
        self.save_log(start_time, "log_positive.txt")
    ...
    def test_totality(self, clear_folder, make_files, start_time):  # t проверка целостности архива
        ...
        res.append(ssh_checkout(data["ip"], data["user"], data["passwd"],
                                ...
                                "Everything is Ok"))
        res.append(ssh_checkout(data["ip"], data["user"], data["passwd"],
                                f'cd {data["folder_out"]}; 7z t arx2.{data["arc_type"]}', "Everything is Ok"))
        self.save_log(start_time, "log_positive.txt")
    ...
    def test_delete(self, clear_folder, make_files, make_subfolder, start_time):  # d удаление из архива
        ...
        res.append(ssh_checkout(data["ip"], data["user"], data["passwd"],
                                ...
                                "Everything is Ok"))
        res.append(ssh_checkout(data["ip"], data["user"], data["passwd"],
                                f'cd {data["folder_out"]}; 7z d arx2.{data["arc_type"]}', "Everything is Ok"))
        self.save_log(start_time, "log_positive.txt")
    ...
    def test_update(self, start_time):  # u - обновление архива
        ...
        assert (ssh_checkout(data["ip"], data["user"], data["passwd"],
                             f'cd {data["folder_in"]}; 7z u {data["folder_out"]}/arx2.{data["arc_type"]}',
